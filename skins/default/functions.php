...
if (!function_exists('magikCreta_user')) {
    function magikCreta_user()
    {
        global $creta_Options;
        if (is_user_logged_in()) {
            global $current_user;
            if (isset($creta_Options['welcome_msg'])) {
                echo esc_attr_e('You are logged in as', 'creta') . '   <b><u>' . esc_attr($current_user->display_name) . '</b></u>';
            }
        } else {
            if (isset($creta_Options['welcome_msg']) && ($creta_Options['welcome_msg'] != '')) {
                //echo htmlspecialchars_decode($creta_Options['welcome_msg']);
                echo esc_attr_e('Register Or Sign In', 'creta');
            }
        }
    }
}
